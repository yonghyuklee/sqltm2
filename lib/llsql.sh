...
if [[ -z "${LOADLBATCH}" ]]; then
    echo "ERROR: $0 not running in LoadLeveler environment."
    exit
fi
...
jobname="${LOADL_STEP_ID}" # the job id used in queries

# *** setup reverse shell ***
reverse_shell="${RRS:-${SQLTM_BASE}/sbin/rrs}"
cert="${CERT:-${SQLTM_BASE}/etc/rrs.pem}"
login_host="${LOGIN_HOST}" # must be set - submission host name is not exported to environment
...
segment="default"
jobinfo="$(llq -m ${jobname})"
#jobname=$(awk -F ':' '/Job Name/{print $2;exit;}') # use LOADL_STEP_ID instead
dispatch_time=$(echo -e "${jobinfo}" | awk -F ' ' '/Dispatch Time/{$1=$2=""; print $0; exit;}')
dispatch_time=$(date -d "${dispatch_time}" "+%s")
time_limit=$(echo -e "${jobinfo}"\
    | awk -F ' ' '/Wall Clk Soft Limit/{print $5; exit;}'\
    | awk -F':' '{ sub("+",":"); if ($4 == "") {$4=$3; $3=$2; $2=$1; $1=0}; print ($1*86400) + ($2 * 3600) + ($3 * 60) + $4 }')
