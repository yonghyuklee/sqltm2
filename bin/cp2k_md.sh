...
inputfile="${step}.inp"
outputfile="${step}.out"
complete_str="${complete_str:-"PROGRAM ENDED AT"}"
#restart_str="${restart_str:-"MD run terminated - exceeded requested execution time"}"
restart_str="${restart_str:-"MD run terminated by external request"}"
...
# check whether task is complete or change no of steps
nstep=$(awk '/STEPS/{print $2}' ${inputfile})
istep=$(awk '/STEP_START_VAL/{print $2}' restart)
rstep=$(expr ${nstep} - ${istep:-0})
if [[ ${rstep} -le 0 ]]; then
    ...
    sed -i "s/STEPS.*/STEPS ${rstep}/" "${inputfile}"
fi
...
if grep -q "${restart_str}" "${outputfile}"; then
    ...
    sql_set "${db}" "${id}" input2 "restart"
elif grep -q "${complete_str}" "${outputfile}"; then
    flag="${flag_complete}"
